...
const Header = () => {
  ...
  const isActiveLink = (href: string) => {
    ...
    if (href.startsWith('/#')) {
      ...
      return pathname === '/' && (window.location.hash === `#${sectionId}` || activeSection === sectionId);
    }
  ...
  return (
    <header
      className={`fixed top-0 w-full z-50 transition-all duration-500 ${
        isScrolled ? 'bg-crown-dark/90 backdrop-blur-md shadow-xl' : 'bg-transparent'
      }`}
    >
                ...
                <li key={item.label}>
                  <a
                    ...
                    className={`transition-colors duration-300 text-base font-medium tracking-wide relative group ${
                      isActive ? 'text-crown-primary' : 'text-crown-white hover:text-crown-primary'
                    }`}
                  >
                    {t(`nav.${item.label}`)}
                    <span
                      className={`absolute -bottom-1 left-0 h-0.5 bg-crown-primary transition-all duration-300 ${
